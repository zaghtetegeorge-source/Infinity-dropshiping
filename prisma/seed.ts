...
// Re-runnable in dev: demo catalog data is replaced on each run, while the
// admin account is only created if it doesn't already exist (so a changed
// password survives re-seeding).
import { PrismaClient } from "@prisma/client";
import bcrypt from "bcryptjs";

const prisma = new PrismaClient();

const ADMIN_EMAIL = process.env.ADMIN_SEED_EMAIL || "admin@infinitystore.com";
...
async function main() {
  ...
  await prisma.adminUser.upsert({
    ...
    update: {},
    create: { email: ADMIN_EMAIL, passwordHash, name: "Store Admin" },
  });
  ...
  console.log(`Admin login: ${ADMIN_EMAIL} / ${ADMIN_PASSWORD} (change this password after first login).`);
}
